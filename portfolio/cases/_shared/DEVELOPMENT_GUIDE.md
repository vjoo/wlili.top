...
| `gallery` | `.screen-section` | `.screen-content`(白卡) > `.screen-info` + `.screen-wall`(2/3列) | 白卡 padding 60px；列宽 2列 474px / 3列 273px；列错落 JS 自动 |
...
| `clients` | `.clients-section` | `.section-label` + `.section-heading` + `.clients-logos`(grid) | margin 5rem（移动 60px）；logo 栅格 auto-fill minmax(210px, 1fr)、gap 0.25rem、margin-top 0.5rem |
